...
describe("bitArrayUtf8SequenceSize", () => {
  test("1 byte UTF-8 sequence", () => {
    ...
    assert.equal(bitArrayUtf8SequenceSize(bitArray, 0), 8);
  });
  ...
  test("3 byte UTF-8 sequence", () => {
    ...
    assert.equal(bitArrayUtf8SequenceSize(bitArray, 0), 24);
  });

  test("4 byte UTF-8 sequence", () => {
    const bitArray = toBitArray([stringBits("💜")]);

    assert.equal(bitArrayUtf8SequenceSize(bitArray, 0), 32);
  });
  ...
  test("fails when there are not enough bits for the sequence", () => {
    ...
    assert.equal(bitArrayUtf8SequenceSize(bitArray, 0), -1);
  });
...
describe("bitArrayUtf8SequenceSizeBranches", () => {
  test("1 byte UTF-8 sequence", () => {
    const bitArray = toBitArray([stringBits("a")]);

    assert.equal(bitArrayUtf8SequenceSizeBranches(bitArray, 0), 8);
  });

  test("2 byte UTF-8 sequence", () => {
    const bitArray = toBitArray([stringBits("é")]);

    assert.equal(bitArrayUtf8SequenceSizeBranches(bitArray, 0), 16);
  });

  test("3 byte UTF-8 sequence", () => {
    const bitArray = toBitArray([stringBits("€")]);

    assert.equal(bitArrayUtf8SequenceSizeBranches(bitArray, 0), 24);
  });

  test("4 byte UTF-8 sequence", () => {
    const bitArray = toBitArray([stringBits("💜")]);

    assert.equal(bitArrayUtf8SequenceSizeBranches(bitArray, 0), 32);
  });

  test("UTF-8 sequence at a non-byte-aligned offset", () => {
    const bitArray = toBitArray([sizedInt(5, 3, true), stringBits("é")]);

    assert.equal(bitArrayUtf8SequenceSizeBranches(bitArray, 3), 16);
  });

  test("multiple UTF-8 sequences can be matched using cumulative offsets", () => {
    const bitArray = toBitArray([stringBits("aé€💜")]);

    const first = bitArrayUtf8SequenceSizeBranches(bitArray, 0);
    const second = bitArrayUtf8SequenceSizeBranches(bitArray, first);
    const third = bitArrayUtf8SequenceSizeBranches(bitArray, first + second);
    const fourth = bitArrayUtf8SequenceSizeBranches(bitArray, first + second + third);
  ...
  test("fails when there are not enough bits for the sequence", () => {
    ...
    assert.equal(bitArrayUtf8SequenceSizeBranches(bitArray, 0), -1);
  });
  ...
  test("fails when start points past the end of the bit array", () => {
    ...
    assert.equal(bitArrayUtf8SequenceSizeBranches(bitArray, 8), -1);
  });
  ...
  test("only measures the sequence at the given position", () => {
    ...
    assert.equal(bitArrayUtf8SequenceSizeBranches(bitArray, 0), 16);
    assert.equal(bitArrayUtf8SequenceSizeBranches(bitArray, 16), 32);
  });
});
